...
value class Token (
    ...
    companion object {
        private const val PLAIN_SECRET_KEY = "kakaocafesecretkeyJaISjiKkjaB"
        ...
        private fun createToken(member: Member): String {
            return Jwts.builder()
                ...
                .claims().add(mapOf("memberId" to member.id, "email" to member.email, "memberStatus" to member.status)).and()
                .subject("user")
    ...
    private fun getClaims(): Claims {
        return runCatching {
            ...
            when (it) {
                is ExpiredJwtException -> throw KakaoCafeException(ErrorCode.EXPIRED_TOKEN)
                else -> throw it
            }
        }.getOrThrow()
    }
